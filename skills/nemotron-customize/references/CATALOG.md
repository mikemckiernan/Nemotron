...
- HARD GUARD (overrides artifact composition): MCQ benchmark/evaluation data
  ...
  MCQ-shaped examples intended for SFT training route to `sdg/persona_mcq` instead.
...
| `sdg/persona_mcq` | Generate persona-grounded English/Hindi MCQ-shaped SFT training data with deduplication and multi-teacher voting. NOT for held-out benchmarks -> those go to `byob/mcq`. | - | `synthetic_jsonl` | `default`, `tiny` | Install with `uv sync --extra data-sdg`; requires Data Designer persona assets and distinct question/answer model endpoints for production. Use a fresh experiment name for smoke runs; outputs are aligned per-teacher OpenAI messages JSONL. Agreement is not factual verification, so audit or separately judge examples before training. |
