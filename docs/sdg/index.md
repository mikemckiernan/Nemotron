...
:link: explanation/planning
:link-type: doc
Planning guidance: what decisions belong to the human before any config is written or any records are generated.
+++
{bdg-success}`1 guide` {bdg-secondary}`concept-focused`
:::
...
| Guide | What You'll Understand |
|---|---|
| {doc}`explanation/planning` | What decisions belong to the human before generating data |
...
| {doc}`how-to/create-greenteme-airlines-dataset` | Adapt the pipeline to a custom domain with a seed file and multiple category dimensions |
