...
- A *Hugging Face* handle, for example `meta-llama/Llama-3.1-8B`, resolved through the *Hugging Face* hub by the loader.
...
The recovery is to extend the path to a specific `iter_*` directory and its `tokenizer/` subdirectory.
