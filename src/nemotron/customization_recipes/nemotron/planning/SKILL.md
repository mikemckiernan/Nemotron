---
name: planning
description: Onboards new users to sovereign AI customization of Nemotron models. Diagnoses the right first move, gates on infrastructure readiness, drafts `env.toml` for Slurm, Lepton, or Run:AI, recommends baseline evaluation of Nemotron Nano V3 30B, and produces a first-day plan. Use this skill when a user is new to Nemotron, wants to build a sovereign model, or needs help choosing between customization and full training.
license: Apache-2.0
---
...
Use this skill when the user is new to the Nemotron customization workflow, says they want to build a sovereign model, is unsure of hardware needs, or needs help choosing the right first step.
...
- Produce a local plan at `sai-onboarding-plan.md` in the same directory as this skill file
...
If there is no working `env.toml` or equivalent profile, stop other progress and use the closest sample under `assets/` to prepare a proposed draft.
...
When choosing the closest sample, use only:
...
1. Refer to the `reference/env-toml.md` file for information about the file requirements
2. Pick the nearest launcher-specific sample under `assets/`
3. Fill in only values that can be inferred safely from the intake and follow-up answers
4. Mark unknown or risky fields plainly
5. Show the full proposed file contents before writing anything
6. Wait for explicit confirmation
7. Write the approved draft only after confirmation
...
Write a local markdown plan to `sai-onboarding-plan.md` in the skill root.
